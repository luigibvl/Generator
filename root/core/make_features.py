...
def make_features_from_r(adapter, path, images_path, roi_name, features_family):
    ...
    features = adapter.extract_features(images_path, roi_name, features_family_list)
    del adapter
    return features
...
def make_features(radiomics_study_id, images_path, ROI_name, features_family):
    ...
    if Features_Model.objects.filter(study_id=radiomics_study_id).exists():
        Features_Model.objects.filter(study_id=radiomics_study_id).update(data=pd_from_r_df)
    else:
        features_model = Features_Model(study_id=radiomics_study_id, data=pd_from_r_df)
        features_model.save()
